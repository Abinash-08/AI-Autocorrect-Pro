...
/* ============================================================
   RENDER RESULT  (v2 — handles enhanced API response)
   ============================================================ */
function renderResult(data, elapsed) {
  const {
    original, corrected,
    mistakes, score,
    grammar_score, spelling_score, readability_score, overall_score,
    changes, explanations, stats
  } = data;

  const displayScore = overall_score ?? score ?? 100;

  // ── Overall score badge ──────────────────────────────────
  scoreVal.textContent = displayScore;
  scoreBadge.style.background =
    displayScore >= 90 ? 'linear-gradient(135deg,#2ECC71,#16A085)' :
    displayScore >= 70 ? 'linear-gradient(135deg,#f39c12,#e67e22)' :
  ...
  const sbPanel = $('scoreBreakdown');
  if (grammar_score != null || spelling_score != null || readability_score != null) {
    sbPanel.style.display = 'block';
    const gs = grammar_score     ?? displayScore;
    const ss = spelling_score    ?? displayScore;
    const rs = readability_score ?? displayScore;
    setTimeout(() => {
      $('sbGrammar').style.width   = gs + '%';
      $('sbSpelling').style.width  = ss + '%';
      $('sbReadability').style.width = rs + '%';
    }, 80);
    $('sbGrammarNum').textContent   = gs;
    $('sbSpellingNum').textContent  = ss;
    $('sbReadabilityNum').textContent = rs;
  } else {
    sbPanel.style.display = 'none';
  }

  // ── Explanations panel ───────────────────────────────────
  const expPanel = $('explanationsPanel');
  const expList  = $('expList');
  if (explanations && explanations.length) {
    expPanel.style.display = 'block';
    expList.innerHTML = explanations.map(e => {
      const reasonClass = getReasonClass(e.reason);
      return `<div class="exp-row">
        <span class="exp-before">${escapeHtml(e.before)}</span>
        <span class="exp-arrow">→</span>
        <span class="exp-after">${escapeHtml(e.after)}</span>
        <span class="exp-reason ${reasonClass}">${escapeHtml(e.reason)}</span>
      </div>`;
    }).join('');
  } else {
  ...
  // ── Text stats row ───────────────────────────────────────
  const tsRow = $('textStatsRow');
  if (stats) {
    tsRow.style.display = 'flex';
    $('tsWords').textContent     = `${stats.words ?? '—'} words`;
    $('tsSentences').textContent = `${stats.sentences ?? '—'} sentences`;
    $('tsChars').textContent     = `${stats.characters ?? '—'} chars`;
    const rt = stats.reading_time_sec;
    $('tsReadTime').textContent  = rt ? (rt >= 60 ? `${Math.ceil(rt/60)}m read` : `${rt}s read`) : '— read';
    $('tsGFixed').textContent    = `${stats.grammar_fixed ?? 0} grammar fixed`;
    $('tsSpFixed').textContent   = `${stats.spelling_fixed ?? 0} spelling fixed`;
  } else {
    tsRow.style.display = 'none';
  }

  // ── Store for downloads ──────────────────────────────────
  state.lastCorrected = { original, corrected, mistakes, score: displayScore };

  outputContent.style.display = 'block';
  outputContent.style.animation = 'none';
  requestAnimationFrame(() => { outputContent.style.animation = ''; });
}

function getReasonClass(reason) {
  if (!reason) return '';
  const r = reason.toLowerCase();
  if (r.includes('spell'))   return 'reason-spell';
  if (r.includes('gram') || r.includes('subject') || r.includes('tense') || r.includes('article') || r.includes('missing') || r.includes('extra')) return 'reason-gram';
  if (r.includes('capital'))  return 'reason-cap';
  if (r.includes('punct'))    return 'reason-punc';
  return '';
}
...
function updateDashboard(data, elapsed) {
  ...
  const sessionScore = data.overall_score ?? data.score ?? 85;

  state.totalWords       += words;
  state.totalCorrections += data.mistakes;
  state.totalScore = Math.round(
    (state.totalScore * state.correctionCount + sessionScore) / (state.correctionCount + 1)
  );
  state.correctionCount++;
  state.totalTimeSaved += parseFloat(elapsed) || 0;
  ...
  // Fill bars
  $('fillWords').style.width       = Math.min(100, state.totalWords / Math.max(state.totalWords, 100) * 100) + '%';
